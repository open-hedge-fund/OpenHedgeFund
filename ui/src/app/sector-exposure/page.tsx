...
import { sectorExposureApi, SectorExposureData, fundApi, FundData } from "@/lib/api";
...
function SectorExposureContent() {
  const [data, setData] = useState<SectorExposureData[]>([]);
  ...
  async function loadData() {
    try {
      ...
      const result = await sectorExposureApi.get(params);
      setData(result);
    } catch {
  ...
  const totalsRow = data.find((d) => d.sector === "Total");
  const dataRows = data.filter((d) => d.sector !== "Total");
