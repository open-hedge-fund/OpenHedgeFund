...
import { custodianApi, CustodianData } from "@/lib/api";
...
function CustodiansContent() {
  const [items, setItems] = useState<CustodianData[]>([]);
  const [searchQuery, setSearchQuery] = useState("");
  const [isCreateModalOpen, setIsCreateModalOpen] = useState(false);
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<CustodianData | null>(null);
  ...
  async function loadItems() {
    try {
      ...
      setError(err.message || "Failed to load custodians");
    } finally {
      setIsLoading(false);
    }
  }
  ...
  const filteredItems = items.filter((item) => {
    ...
    return (
      ...
      item.custodian_code?.toLowerCase().includes(q) ||
      item.account_number?.toLowerCase().includes(q) ||
      item.email?.toLowerCase().includes(q)
    );
  });

  async function handleCreate(data: Partial<CustodianData>) {
    try {
      setError("");
      await custodianApi.create(data);
      await loadItems();
      setIsCreateModalOpen(false);
    } catch (err: any) {
      setError(err.message || "Failed to create custodian");
      throw err;
    }
  }

  async function handleUpdate(id: number, data: Partial<CustodianData>) {
    try {
      setError("");
      await custodianApi.update(id, data);
      await loadItems();
      setIsEditModalOpen(false);
      setSelectedItem(null);
    } catch (err: any) {
      setError(err.message || "Failed to update custodian");
      throw err;
    }
  }

  function handleEdit(item: CustodianData) {
  ...
  async function handleDelete(id: number) {
    ...
    try {
      setError("");
      await custodianApi.delete(id);
      await loadItems();
    } catch (err: any) {
      setError(err.message || "Failed to delete custodian");
    }
  }
    ...
    <div>
      <div className="files-card">
        <div className="files-header">
          <div>
            <h2>Custodians</h2>
            <p>Manage your custodial service providers</p>
          </div>
        ...
        <div className="files-search">
          <input
            type="text"
            placeholder="Search custodians by name, code, account number, or email..."
            value={searchQuery}
        ...
        <div className="files-table-wrapper">
          {isLoading ? (
            ...
            <div className="files-empty">No custodians found</div>
          ) : (
...
function CreateModal({ onClose, onCreate, error }: { onClose: () => void; onCreate: (data: Partial<CustodianData>) => Promise<void>; error: string }) {
  const [formData, setFormData] = useState<Partial<CustodianData>>({
    custodian_name: "",
    custodian_code: "",
    account_number: "",
    email: "",
        ...
        <form className="modal-form" onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            <input
              type="text"
              className="form-input"
              value={formData.custodian_code || ""}
              onChange={(e) => setFormData({ ...formData, custodian_code: e.target.value })}
              required
            />
          </div>
...
function EditModal({ item, onClose, onUpdate, error }: { item: CustodianData; onClose: () => void; onUpdate: (id: number, data: Partial<CustodianData>) => Promise<void>; error: string }) {
  const [formData, setFormData] = useState<Partial<CustodianData>>({
