...
import { currencyApi, CurrencyData } from "@/lib/api";
...
function CurrenciesContent() {
  ...
  const [selectedItem, setSelectedItem] = useState<CurrencyData | null>(null);
  ...
  const filteredItems = items.filter((item) => {
    ...
    return (
      item.ccy?.toLowerCase().includes(q) ||
      item.ccy_des?.toLowerCase().includes(q)
    );
  });

  async function handleCreate(data: Partial<CurrencyData>) {
    try {
      setError("");
      await currencyApi.create(data);
      await loadItems();
  ...
  async function handleUpdate(id: number, data: Partial<CurrencyData>) {
    try {
      setError("");
  ...
  function handleEdit(item: CurrencyData) {
    ...
    <div>
      <div className="files-card">
        <div className="files-header">
          <div>
            <h2>Currencies</h2>
            <p>Manage currency codes and descriptions</p>
          </div>
        ...
        <div className="files-search">
          <input
            type="text"
            placeholder="Search currencies by code or description..."
            value={searchQuery}
        ...
        <div className="files-table-wrapper">
          {isLoading ? (
            ...
            <div className="files-empty">No currencies found</div>
          ) : (
...
function CreateModal({ onClose, onCreate, error }: { onClose: () => void; onCreate: (data: Partial<CurrencyData>) => Promise<void>; error: string }) {
  const [formData, setFormData] = useState<Partial<CurrencyData>>({
    ccy: "",
    ccy_des: "",
  });
        ...
        <form className="modal-form" onSubmit={handleSubmit}>
          <div className="form-group">
            ...
            <input
              ...
              value={formData.ccy || ""}
              onChange={(e) => setFormData({ ...formData, ccy: e.target.value })}
              required
            />
          </div>
          ...
          <div className="form-group">
            ...
            <input
              type="text"
              className="form-input"
              value={formData.ccy_des || ""}
              onChange={(e) => setFormData({ ...formData, ccy_des: e.target.value })}
              required
            />
          </div>
...
function EditModal({ item, onClose, onUpdate, error }: { item: CurrencyData; onClose: () => void; onUpdate: (id: number, data: Partial<CurrencyData>) => Promise<void>; error: string }) {
  const [formData, setFormData] = useState<Partial<CurrencyData>>({
    ccy: item.ccy,
    ccy_des: item.ccy_des,
  });
  const [isSubmitting, setIsSubmitting] = useState(false);
