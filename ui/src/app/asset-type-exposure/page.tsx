...
import { assetTypeExposureApi, AssetTypeExposureData, fundApi, FundData } from "@/lib/api";
...
function AssetTypeExposureContent() {
  const [data, setData] = useState<AssetTypeExposureData[]>([]);
  ...
  async function loadData() {
    try {
      ...
      const result = await assetTypeExposureApi.get(params);
      setData(result);
    } catch {
  ...
  const totalsRow = data.find((d) => d.asset_type === "Total");
  const dataRows = data.filter((d) => d.asset_type !== "Total");
    ...
    <div>
      <div className="files-card">
        <div className="files-header">
          <div>
            <h2>Asset Type Exposure</h2>
            <p>Exposure by asset type</p>
          </div>
        </div>
