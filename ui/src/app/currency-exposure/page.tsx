...
import { currencyExposureApi, CurrencyExposureData, fundApi, FundData } from "@/lib/api";
...
function CurrencyExposureContent() {
  const [data, setData] = useState<CurrencyExposureData[]>([]);
  ...
  async function loadData() {
    try {
      ...
      const result = await currencyExposureApi.get(params);
      setData(result);
    } catch {
  ...
  const totalsRow = data.find((d) => d.currency === "Total");
  const dataRows = data.filter((d) => d.currency !== "Total");
